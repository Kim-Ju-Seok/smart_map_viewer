...
function refreshView() {
  ...
  statToday.textContent = filtered.filter(r => r.date === todayStr).length;
...
function initMap() {
  ...
  map = new naver.maps.Map(mapContainer, {
    ...
    zoomControl: true,
    zoomControlOptions: {
      position: naver.maps.Position.RIGHT_CENTER // 와이드 화면에서 조작이 쉽도록 우측 중앙 배치
    },
    logoControl: false, // 하단 정보를 더 넓게 쓰기 위해 로고 숨김 처리 가능
    mapDataControl: false
  });

  naver.maps.Event.addListener(map, 'click', resetToDongView);

  // 창 크기 변경 시 지도가 잘리거나 위치가 어긋나지 않도록 사이즈 재설정
