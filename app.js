  ...
  for (const item of items) {
    ...
    if (coords) {
      const m = new naver.maps.Marker({
        ...
        title: item.address,
        icon: {
          content: `<div style="color:#ff6a3d; font-size:24px;">📍</div>`,
      ...
      icon: {
        content: `
          <div style="width:${size}px; height:${size}px; 
               ...
               box-shadow: 0 8px 16px rgba(255, 106, 61, 0.2); 
               cursor:pointer; transition: transform 0.2s cubic-bezier(0.175, 0.885, 0.32, 1.275);">
            <div style="font-size:10px; font-weight:700; opacity:0.7;">${dongName}</div>
            <div style="font-size:18px; font-weight:800;">${data.count}</div>
          </div>`,
...
function refreshView() {
  ...
  // 2. [오늘 신규] 필터에 영향을 받지 않도록 전체 원본 데이터(rawRows)에서 오늘 날짜만 계산
  const todayStr = new Date().toISOString().slice(0, 10);
  const realTodayCount = rawRows.filter(r => r.date === todayStr).length;
  statToday.textContent = realTodayCount;

  listCount.textContent = `${filtered.length} 건`;

  // 3. 테이블 행 렌더링 (CSS에서 지정한 .table-wrap 안에서 스크롤 됨)
  tableBody.innerHTML = filtered.map(row => `
    <tr>
      <td style="font-weight:600; color:#1e293b;">${row.date}</td>
      <td><span style="background:#f1f5f9; padding:4px 8px; border-radius:6px;">${row.dong}</span></td>
      <td>${row.offlinePath}</td>
      <td style="color:#64748b; font-size:12px;">${row.region}</td>
    </tr>`).join("");
...
async function loadData() {
  mapStatus.innerHTML = `<span style="color:#26d0ce; font-weight:600;">📡 데이터를 불러오는 중...</span>`;
  try {
    ...
    refreshView();
    mapStatus.textContent = "✅ 최신 데이터";
    setTimeout(() => { mapStatus.textContent = ""; }, 3000);
  } catch (e) { 
    console.error(e);
    mapStatus.innerHTML = `<span style="color:#ff6a3d;">❌ 연결 실패</span>`; 
  }
}
...
function initCalendars() {
  ...
  flatpickr("#date-end", commonConfig);
}

// 이벤트 리스너 등록
[searchText].forEach(el => el.addEventListener("input", refreshView));
